...
var webflowEnv = (() => {
  ...
  function rollReveal(selector3 = ".roll_reveal", duration = 2) {
    gsapWithCSS.set(selector3, {
  ...
  function rollRevealReverse(selector3 = ".roll_reveal_revese", duration = 2) {
    gsapWithCSS.set(selector3, {
  ...
  function fadeIn() {
    gsapWithCSS.from(".fade_in", {
      ...
      scrollTrigger: {
        ...
        toggleActions: "play none none none",
        // Play on enter, no other actions
        markers: true
        // Optional: Visual debugging (remove in production)
      }
    });
  }
  ...
  function flipReveal() {
    const flipReveal2 = gsapWithCSS.timeline({
      scrollTrigger: {
        trigger: ".flip_reveal",
        start: "top 80%",
        end: "top 30%",
    ...
    let num = 3;
    flipReveal2.to(".flip_reveal img", {
      rotationY: `+=${360 * num}`,
      stagger: 0.2,
      duration: 2,
      ease: "back.out(1.7)",
  ...
  function slideInLeft() {
    gsapWithCSS.from(".slide_in_left", {
      x: -300,
      opacity: 0,
      duration: 2,
      ease: "power3.out",
  ...
  function slideInRight() {
    gsapWithCSS.from(".slide_in_right", {
      x: 300,
      opacity: 0,
      duration: 2,
      ease: "power3.out",
      scrollTrigger: {
        trigger: ".slide_in_right",
        // Element that triggers the animation
        start: "top 75%",
  ...
  function staggerItemScroll() {
    gsapWithCSS.from(".stagger-item", {
      scrollTrigger: {
  ...
  function slideInLeftScroll() {
    gsapWithCSS.from(".slide-in-left", {
      scrollTrigger: {
  ...
  function slideInRightScroll() {
    gsapWithCSS.from(".slide-in-right", {
      scrollTrigger: {
  ...
  function horizontalScroll() {
    ...
    const panels = gsapWithCSS.utils.toArray(".panel_horizontal");
  ...
  function threePanelfade() {
    const panels = document.querySelectorAll(".panel");
    let tl = gsapWithCSS.timeline({
      scrollTrigger: {
        trigger: ".panel-wrapper",
        // wrap all your panels in a parent
        start: "top top",
        end: `+=${panels.length * 200}%`,
        // 100% per panel (adjust if you add/remove panels)
        scrub: true,
        pin: true
      }
    });
  ...
  function getRotationAtBezierT(segment, i, t) {
    ...
    b += (c - b) * t;
    a += (b - a) * t;
    return _round7(_atan22(b + (c + (segment[i + 7] - c) * t - b) * t - a, x) * _RAD2DEG2);
  }
  ...
  function getPositionOnPath(rawPath, progress, includeAngle, point) {
    ...
    i = ~~(i / resolution) * 6;
    a = segment[i];
    result.x = _round7((t * t * (segment[i + 6] - a) + 3 * inv * (t * (segment[i + 4] - a) + inv * (segment[i + 2] - a))) * t + a);
    result.y = _round7((t * t * (segment[i + 7] - (a = segment[i + 1])) + 3 * inv * (t * (segment[i + 5] - a) + inv * (segment[i + 3] - a))) * t + a);
    if (includeAngle) {
  ...
  function arcToSegment(lastX, lastY, rx, ry, angle, largeArcFlag, sweepFlag, x, y) {
    ...
    if (radiiCheck > 1) {
      rx = _sqrt3(radiiCheck) * rx;
      ry = _sqrt3(radiiCheck) * ry;
    }
    var rx_sq = rx * rx, ry_sq = ry * ry, sq = (rx_sq * ry_sq - rx_sq * y1_sq - ry_sq * x1_sq) / (rx_sq * y1_sq + ry_sq * x1_sq);
    if (sq < 0) {
      sq = 0;
    }
    var coef = (largeArcFlag === sweepFlag ? -1 : 1) * _sqrt3(sq), cx1 = coef * (rx * y1 / ry), cy1 = coef * -(ry * x1 / rx), sx2 = (lastX + x) / 2, sy2 = (lastY + y) / 2, cx = sx2 + (cosAngle * cx1 - sinAngle * cy1), cy = sy2 + (sinAngle * cx1 + cosAngle * cy1), ux = (x1 - cx1) / rx, uy = (y1 - cy1) / ry, vx = (-x1 - cx1) / rx, vy = (-y1 - cy1) / ry, temp = ux * ux + uy * uy, angleStart = (uy < 0 ? -1 : 1) * Math.acos(ux / _sqrt3(temp)), angleExtent = (ux * vy - uy * vx < 0 ? -1 : 1) * Math.acos((ux * vx + uy * vy) / _sqrt3(temp * (vx * vx + vy * vy)));
  ...
  function pointsToSegment(points, curviness) {
    ...
    for (i = 2; i < l; i += 2) {
      ...
      dx2 = nextX - x;
      dy2 = nextY - y;
      r1 = _sqrt3(dx1 * dx1 + dy1 * dy1);
      r2 = _sqrt3(dx2 * dx2 + dy2 * dy2);
      r3 = _sqrt3(Math.pow(dx2 / r2 + dx1 / r1, 2) + Math.pow(dy2 / r2 + dy1 / r1, 2));
  ...
  var _getDocScrollTop = function _getDocScrollTop2() {
    return _win6.pageYOffset || _doc6.scrollTop || _docElement2.scrollTop || _body3.scrollTop || 0;
  };
  var _getDocScrollLeft = function _getDocScrollLeft2() {
    return _win6.pageXOffset || _doc6.scrollLeft || _docElement2.scrollLeft || _body3.scrollLeft || 0;
  };
  var _svgOwner = function _svgOwner2(element) {
    return element.ownerSVGElement || ((element.tagName + "").toLowerCase() === "svg" ? element : null);
  };
  var _isFixed = function _isFixed2(element) {
    if (_win6.getComputedStyle(element).position === "fixed") {
      return true;
    }
    ...
    if (svg) {
      ...
      if (_hasOffsetBug) {
        ...
        while (b && (b = b.parentNode) && b !== m && b.parentNode) {
          if ((_win6.getComputedStyle(b)[_transformProp3] + "").length > 4) {
            x = b.offsetLeft;
            y = b.offsetTop;
            b = 0;
          }
      ...
      cs = _win6.getComputedStyle(element);
  ...
  var _align = function _align2(rawPath, target, _ref) {
    ...
    if (align) {
      if (align === "self" || (alignTarget = _toArray4(align)[0] || target) === target) {
  ...
  var MotionPathPlugin = {
    ...
    register: function register2(core, Plugin, propTween) {
      ...
      _reverting4 = gsap7.core.reverting || function() {
      };
  ...
  gsapWithCSS.registerPlugin(ScrollTrigger2, DrawSVGPlugin, MotionPathPlugin);
  function svgScroll() {
    gsapWithCSS.set(".ball01", { x: -5, y: 0, autoAlpha: 1 });
    const pulses = gsapWithCSS.timeline();
    pulses.fromTo(
      ".ball02, .text01",
      { autoAlpha: 0, scale: 0 },
      { autoAlpha: 1, scale: 2, transformOrigin: "center", ease: "elastic(2.5, 1)" },
      0.2
    );
    pulses.fromTo(
      ".ball03, .text02",
    ...
    const main = gsapWithCSS.timeline({
      scrollTrigger: {
        trigger: "#svg-stage",
        scrub: true,
        start: "top center",
    ...
    main.from(".theLine", { drawSVG: 0, duration: 4 }).to(".ball01", {
      motionPath: {
        path: ".theLine",
        align: ".theLine",
        alignOrigin: [0.5, 0.5],
  ...
  function Contextual() {
    ScrollTrigger2.create({
      ...
      onEnter: () => {
        const element = document.getElementsByClassName("Contextual");
  ...
  function vidOnSnap() {
    ...
    function initScrub() {
      ...
      ScrollTrigger2.addEventListener("scrollEnd", () => {
        if (ScrollTrigger2.isInViewport(vid)) {
          vid.play();
        } else {
          vid.pause();
        }
  ...
  // src/index.ts
  gsapWithCSS.registerPlugin(ScrollTrigger2, TextPlugin, SplitText);
