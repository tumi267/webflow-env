...
(() => {
  ...
  var DrawSVGPlugin_exports = {};
  __export(DrawSVGPlugin_exports, {
  ...
  var _svgPathExp, _numbersExp, _scientific, _selectorExp, _DEG2RAD2, _RAD2DEG2, _sin2, _cos2, _abs2, _sqrt3, _atan22, _largeNum, _isString5, _isNumber5, _isUndefined3, _temp, _temp2, _roundingNum, _wrapProgress, _round7, _roundPrecise3, _splitSegment, _getSampleIndex, _reverseRawPath, _copyMetaData, _appendOrMerge, _createPath, _typeAttrs, _attrToObj;
  ...
  var MotionPathPlugin_exports = {};
  __export(MotionPathPlugin_exports, {
    MotionPathPlugin: () => MotionPathPlugin,
    default: () => MotionPathPlugin
  });
  var _xProps, _yProps, _DEG2RAD3, gsap5, PropTween2, _getUnit, _toArray3, _getStyleSaver4, _reverting4, _getGSAP7, _populateSegmentFromArray, _getPropNum, _relativize, _segmentToRawPath, _emptyFunc3, _numExp3, _originToPoint, _getAlignMatrix, _align, _addDimensionalPropTween, _sliceModifier, MotionPathPlugin;
    ...
    "node_modules/gsap/MotionPathPlugin.js"() {
      ...
      MotionPathPlugin = {
        ...
        register: function register2(core, Plugin, propTween) {
          ...
          _reverting4 = gsap5.core.reverting || function() {
          };
  ...
  async function horizontalScroll(id) {
    const { gsap: gsap6 } = await Promise.resolve().then(() => (init_gsap(), gsap_exports));
    const { ScrollTrigger: ScrollTrigger3 } = await Promise.resolve().then(() => (init_ScrollTrigger(), ScrollTrigger_exports));
    gsap6.registerPlugin(ScrollTrigger3);
  ...
  async function threePanelFade(id) {
    ...
    return () => ScrollTrigger3.getAll().forEach((st) => st.kill());
  }
  ...
  async function svgScroll(id) {
    const { gsap: gsap6 } = await Promise.resolve().then(() => (init_gsap(), gsap_exports));
    const { ScrollTrigger: ScrollTrigger3 } = await Promise.resolve().then(() => (init_ScrollTrigger(), ScrollTrigger_exports));
    const { DrawSVGPlugin: DrawSVGPlugin2 } = await Promise.resolve().then(() => (init_DrawSVGPlugin(), DrawSVGPlugin_exports));
    const { MotionPathPlugin: MotionPathPlugin2 } = await Promise.resolve().then(() => (init_MotionPathPlugin(), MotionPathPlugin_exports));
    gsap6.registerPlugin(ScrollTrigger3, DrawSVGPlugin2, MotionPathPlugin2);
    gsap6.set(".ball01", { x: -5, y: 0, autoAlpha: 1 });
    const pulses = gsap6.timeline();
    pulses.fromTo(
      ".ball02, .text01",
      { autoAlpha: 0, scale: 0 },
      {
  ...
  async function vidOnSnap() {
    ...
    function initScrub() {
      ...
      ScrollTrigger3.addEventListener("scrollEnd", () => {
        if (ScrollTrigger3.isInViewport(vid)) {
          vid.play();
        } else {
          vid.pause();
        }
