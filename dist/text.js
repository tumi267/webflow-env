...
(() => {
  ...
  var SplitText_exports = {};
  __export(SplitText_exports, {
    SplitText: () => SplitText,
    default: () => SplitText
  });
  var gsap4, _fonts, _coreInitted4, _initIfNecessary, _charSegmenter, _toArray2, _elements, _emptyArray3, _context4, _spacesRegEx, _emojiSafeRegEx, _emptyBounds, _stretchToFitSpecialChars, _disallowInline, _insertNodeBefore, _getWrapper, _getLineWrapper, _splitWordsAndCharsRecursively, _SplitText, SplitText;
  var init_SplitText = __esm({
    "node_modules/gsap/SplitText.js"() {
      ...
      _toArray2 = (r) => typeof r === "string" ? _toArray2(document.querySelectorAll(r)) : "length" in r ? Array.from(r) : [r];
      _elements = (targets) => _toArray2(targets).filter((e) => e instanceof HTMLElement);
      _emptyArray3 = [];
      _context4 = function() {
      };
      _spacesRegEx = /\s+/g;
  ...
  async function initLineAnimations(id) {
    const { gsap: gsap5 } = await Promise.resolve().then(() => (init_gsap(), gsap_exports));
    const { ScrollTrigger: ScrollTrigger3 } = await Promise.resolve().then(() => (init_ScrollTrigger(), ScrollTrigger_exports));
    const { SplitText: SplitText2 } = await Promise.resolve().then(() => (init_SplitText(), SplitText_exports));
    document.addEventListener("DOMContentLoaded", () => {
      const split = SplitText2.create(`#${id}`, {
        type: "lines"
      });
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      words.forEach((word, i) => {
        tl.from(word, {
          y: -100,
          opacity: 0,
          rotation: gsap5.utils.random(-80, 80),
          duration: 0.7,
          ease: "back"
        }, i * 0.15);
      });
  ...
  async function initCharAnimations(id) {
    ...
    const { ScrollTrigger: ScrollTrigger3 } = await Promise.resolve().then(() => (init_ScrollTrigger(), ScrollTrigger_exports));
    const { SplitText: SplitText2 } = await Promise.resolve().then(() => (init_SplitText(), SplitText_exports));
    gsap5.registerPlugin(ScrollTrigger3, SplitText2);
    document.addEventListener("DOMContentLoaded", () => {
      const split = SplitText2.create(`#${id}`, { type: "chars" });
  ...
  async function initMaskAnimation() {
    ...
    gsap5.registerPlugin(ScrollTrigger3);
    document.addEventListener("DOMContentLoaded", () => {
      let char = SplitText2.create(".headline", {
        type: "chars"
      });
      let word = SplitText2.create(".sub_headline", {
        type: "words"
      });
      let line = SplitText2.create(".line_amination", {
        type: "lines"
      });
      SplitText2.create(".line_amination_mask", {
        type: "words,lines",
        linesClass: "line",
        autoSplit: true,
        aria: "hidden",
      ...
      let tl = gsap5.timeline({ defaults: { duration: 1, autoAlpha: 0, y: -100 } });
